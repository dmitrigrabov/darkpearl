...
export async function listInventory(
  ...
  if (lowStock) {
    const filteredData = inventoryData.filter(item => item.quantity_available <= item.reorder_point)
...
export async function getInventory(
  client: Client,
  id: string
): Promise<PostgrestSingleResponse<InventoryWithRelations>> {
  return await client
    .from('inventory')
    .select(
      `
...
): Promise<PostgrestSingleResponse<Inventory>> {
  return await client
    .from('inventory')
    .insert({
